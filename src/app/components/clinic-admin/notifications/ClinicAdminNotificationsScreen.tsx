...
import { Search, Filter, X, Bell, Calendar, AlertCircle, CheckCircle, FileText, DollarSign, Clock, Users, Building2 } from "lucide-react";
...
interface ClinicAdminNotificationsScreenProps {
...
export function ClinicAdminNotificationsScreen({
  notifications,
  onNavigate,
  onViewNotification,
  onMarkAsRead,
  onMarkAllAsRead,
  onLogout,
}: ClinicAdminNotificationsScreenProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [showFilters, setShowFilters] = useState(false);
  const [typeFilter, setTypeFilter] = useState<"all" | "reminder" | "alert" | "message" | "action">("all");
  const [categoryFilter, setCategoryFilter] = useState<"all" | "appointment" | "payment" | "system" | "user" | "branch" | "general">("all");
          ...
          {showFilters && (
            <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg p-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {["all", "appointment", "payment", "system", "user", "branch", "general"].map((category) => (
                    <button
                      key={category}
