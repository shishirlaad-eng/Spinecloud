...
import { X, MapPin, User, Calendar, ChevronRight, FileText, ArrowLeft } from "lucide-react";
...
type RescheduleStep = "branch" | "service" | "provider" | "datetime";
...
export function RescheduleDrawer({
  ...
  const [step, setStep] = useState<RescheduleStep>("branch");
          ...
          {step === "service" && (
            <div className="space-y-3">
              {services.map(service => (
                <button
                  key={service}
                  onClick={() => { setSelectedService(service); setStep("provider"); }}
                  className={`w-full text-left p-4 rounded-xl border transition-all ${
                    selectedService === service ? "border-primary-500 bg-primary-50 dark:bg-primary-950/20" : "border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50"
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <FileText className="w-5 h-5 text-neutral-400 mt-1" />
                    <div className="flex-1">
                      <h4 className="font-semibold text-neutral-900 dark:text-white">{service}</h4>
                    </div>
                  </div>
                </button>
              ))}
